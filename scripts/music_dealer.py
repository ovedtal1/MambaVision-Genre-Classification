...
class MusicDealer:
    def __init__(self, weight_path, model, size=128):
        ...
        self.model.load_state_dict(torch.load(weight_path, map_location='cuda:0'))
        self.model = model.eval().cuda(0)
    ...
    def get_genre(self, audio_path):
        ...
        for i, data in enumerate(self.audio_loader(audio_path)):
            with torch.no_grad():
                data = torch.FloatTensor(data).view(1, 1, self.index, self.index).cuda(0)  # resize to fit it in model
